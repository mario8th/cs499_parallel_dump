...
int main(int argc, char *argv[])
{
   ...
   errCode=cudaMalloc((struct pointData**)&dev_data, sizeof(struct pointData)*count)
   ...
   errCode=cudaMemcpy( dev_data, data_ptr, sizeof(struct pointData)*count, cudaMemcpyHostToDevice);
   if(errCode != cudaSuccess) {
   ...
   const unsigned int totalBlocks=ceil(count*1.0/1024.0);
