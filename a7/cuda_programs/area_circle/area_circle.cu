...
//remove aromic adds to see performance difference 
...
////////////////////////////	
	...
	*countNumInCircle=0;
	...
	//execute kernel
	areaCircle<<<totalBlocks,1024>>>(dev_PointList, dev_countNumInCircle);
	...
	//copy count in circle from device to host -- total in circle
	errCode=cudaMemcpy( countNumInCircle, dev_countNumInCircle, sizeof(unsigned long long int), cudaMemcpyDeviceToHost);
	if(errCode != cudaSuccess) {
